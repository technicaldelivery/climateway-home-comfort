...
- Keep the privacy consent banner on the far right of the viewport; a stored choice must not flash on navigation.
- The footer Legal column should include Cookie settings that reopen the privacy consent banner.
...
- Resend cannot send from `climateway.co.uk` until that domain is verified; production currently uses `RESEND_FROM` `noreply@resend.asqrd.dev`.
...
- 1Password mounts `.env` as a FIFO that makes Vite restart in a loop; Vite ignores that file, and local secrets belong in `.env.local`.
